...
class SAC:
    ...
    def __init__(self, env: gym.Env, lr_actor=3e-5, lr_critic=3e-4, actor_units=(64, 64), critic_units=(64, 64),
        ...
        # env
        self.env = env
        self.obs_dims, self.act_dims = env.observation_space.shape, env.action_space.shape
        ...
        self.replay_buffer = PlainReplayBuffer(self.obs_dims[0], self.act_dims[0], self.buffer_size)
    ...
    def act(self, state, test=False, use_random=False):
        ...
        if use_random:
            a = tf.random.uniform(shape=(1, self.act_dims[0]), minval=-1, maxval=1, dtype=tf.float64)
        else:
