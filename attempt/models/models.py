...
def Critic_gen(state_size, action_size, hidden_layers):
    ...
    for i, j in enumerate(hidden_layers[:-1]):
        ...
        x = Dense(j, activation='relu', kernel_initializer=tf.keras.initializers.Orthogonal(gain=tf.sqrt(2.0)))(x)
...
def Actor_gen(state_size, action_size, hidden_layers, action_mult=1):
    ...
    x = Dense(action_size, activation='tanh', kernel_initializer=tf.keras.initializers.Orthogonal(gain=tf.sqrt(2.0)))(x)
